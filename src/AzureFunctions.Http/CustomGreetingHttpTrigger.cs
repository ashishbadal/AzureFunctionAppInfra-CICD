...
namespace AB.Demo
{
    public static class CustomGreetingHttpTrigger
    {
        ...
        public static async Task<IActionResult> Run(
            [HttpTrigger(AuthorizationLevel.Function,  nameof(HttpMethods.Get), nameof(HttpMethod.Post),Route = "CustomGreetingHttpTrigger/{greeting?}")] HttpRequestMessage req,
             string greeting, 
            ILogger log)
        {
            ...
            {
                greeting = greeting ?? "Hello default";
                return new OkObjectResult($"{greeting}- {name}");
            }
